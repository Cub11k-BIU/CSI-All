...
/**
 * @brief Remove Pokemon by ID and print deletion info.
 * @param root BST root
 * @param id the ID to remove
 * @return updated BST root
 * Why we made it: All deletions include at least two actions: remove+print.
 */
...
#define QUEUE_CAPACITY_REALLOC_MULTIPLIER 2

/**
 * @brief Initialize a NodeQueue with given capacity.
...
/**
 * @brief Add a PokemonNode pointer to NodeQueue, realloc if needed.
 * @param nq pointer to NodeQueue
 * @param node pointer to the node
 * Why we made it: We want a dynamic queue of BST nodes for sorting.
 */
void insertNodeQueue(NodeQueue *nq, PokemonNode *node);

/**
 * @brief Pop a PokemonNode pointer from NodeQueue.
 * @param nq pointer to NodeQueue
 * @return PokemonNode
 * Why we made it: We want a dynamic queue of BST nodes for sorting.
 */
...
/**
 * @brief Check if NodeQueue is empty.
 * @param nq pointer to NodeQueue
 * @return 0, 1
 * Why we made it: We want a dynamic queue of BST nodes for sorting.
 */
...
/**
 * @brief Free a NodeQueue.
 * @param nq Pointer to NodeQueue
 * Why we made it: We need a standard way to free the NodeQueue after use.
 */
void freeNodeQueue(NodeQueue *nq);

/**
 * @brief Recursively collect all nodes from the BST into a NodeQueue.
 * @param root BST root
 * @param nq pointer to NodeQueue
...
typedef int (*OwnerNodeComparator)(const void *, const void *);
...
/**
 * @brief Split the doubly linked list into two halves.
 * @param head pointer to the first element in the list
 * @return Pointer to the first element in second half
 * Why we made it: It is one of key components of a recursive merge sort.
 */
OwnerNode *splitLinkedList(OwnerNode *head);

/**
 * @brief Merge two sorted doubly linked lists.
 * @param first pointer to the first half of the list to merge
 * @param second pointer to the second half of the list to merge
 * @return Pointer to the first element in merged list
 * Why we made it: It is one of key components of a recursive merge sort.
 */
OwnerNode *mergeLinkedLists(OwnerNode *first, OwnerNode *second, OwnerNodeComparator cmp);

/**
 * @brief Merge sort (stable) a doubly linked list with a generic comparator.
 * @param head pointer to the first element of the list to sort
 * @param cmp pointer to a comparator function
 * @return Pointer to the first element in a sorted list
 * Why we made it: Sorting the doubly linked lists with merge sort is very effective.
 */
OwnerNode *mergeSort(OwnerNode *head, OwnerNodeComparator cmp);

/**
 * @brief Comparator for owners names to be used in merge sort.
 * @param a pointer to the first element to compare
 * @param a pointer to the second element to compare
 * @return -1, 0, +1
 * Why we made it: Another demonstration of generic function-pointers usage.
 */
...
/**
 * @brief Print owners with indices starting from 1.
 * Why we made it: We often need to print owners for user to choose from.
 */
...
#define PRINT_DIRECTION_FORWARD 'f'
#define PRINT_DIRECTION_BACKWARDS 'b'
...
// Added dummy pokemon with id 0 for index padding, IMO better than doing pokedex[id - 1]
static const PokemonData pokedex[] = {
    {0, "Dummy", GRASS, 0, 0, CANNOT_EVOLVE},
...
// Array of Pokemon starters
// Dummy NULL for index padding, imo better than doing choice - 1
static const PokemonData *starters[] = {
        NULL,
        &pokedex[1],
        &pokedex[4],
        &pokedex[7],
